...
Current code: /mnt/project/Edge_Functions_-_create-loan.txt

This function needs ENHANCEMENT:
- Add establishment fee calculation (tiered by loan amount)
- Create fee_applications record for establishment fee
- Create loan_balances record (initialize balances)
- Create first repayment_schedule entry
- Log ADV (advance) transaction
- Log EST (establishment fee) transaction

Receives:
- client_id (UUID)
- product_id (UUID)
- loan_amount (decimal)
- establishment_fee (decimal, optional - user can override auto-calc)
- start_date (date)
- term (varchar: 'Weekly', 'Fortnightly', 'Monthly')
- is_consolidation (boolean, optional)
- source (varchar: 'new' or 'consolidation')
